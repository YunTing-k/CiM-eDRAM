...
if __name__ == '__main__':
    ...
    repeat_num = 5  # 一种情况下测试多少次
    time_num1 = 3
    time_num2 = 8
    time_num3 = 3
    time_num = time_num1 + time_num2 + time_num3
    time_seq = np.append(np.logspace(0, np.log10(20000), num=time_num1),
                         np.logspace(np.log10(30000), np.log10(90000), num=time_num2))  # 仿真时间序列
    time_seq = np.append(time_seq, np.logspace(np.log10(100000), np.log10(486500), num=time_num3))  # 仿真时间序列
    variation_set = ['26%']  # 加入噪声进行测试
    target_dataset_name = ['MNIST', 'CIFAR10']
    target_num = len(target_dataset_name)
    if train_acc:
        acc_array = np.zeros((target_num, time_num, repeat_num, 2))  # 准确度结果 [A]种网络 [B]种不一致性 [C]次重复 [D]个数据(测试集/训练集)
    else:
        acc_array = np.zeros((target_num, time_num, repeat_num))  # 准确度结果 [A]种网络 [B]种不一致性 [C]次重复
    """Program start"""
    ...
    gpm.set_param('Include_Noise', True)  # 是否考虑噪声
    ...
    for net_type in range(target_num):  # 遍历网络
        ...
        for net_time in range(time_num):
            gpm.set_param('Device_File_nonideal', './Device/ZnO/4bit/nonideal/' + variation_set[0] + '/')
            ...
            if noise_aware:
                ...
                quantized_tensor_tag, quant_scale_sets, quant_bias_sets \
                    = NN_Quantization.quantization_drop(net, get('Device_File_nonideal'),
                                                        get('Quantization_Method'), time_seq[net_time])
            else:
                quantized_net, quantized_weight_lut_sets, quantized_tag_lut_sets, \
                quantized_tensor_tag, quant_scale_sets, quant_bias_sets \
                    = NN_Quantization.quantization_drop(net, get('Device_File_ideal'),
                                                        get('Quantization_Method'), time_seq[net_time])
            for itr in range(repeat_num):
                ...
                if train_acc:
                    ...
                    acc_array[net_type, net_time, itr, 0] = 100 * sum(class_correct_test) / sum(class_total_test)
                    class_correct_train, class_total_train = NN_Inference.inference(noise_net, True, 'Quant')
                    acc_array[net_type, net_time, itr, 1] = 100 * sum(class_correct_train) / sum(class_total_train)
                else:
                    class_correct_test, class_total_test = NN_Inference.inference(noise_net, False, 'Quant')
                    acc_array[net_type, net_time, itr] = 100 * sum(class_correct_test) / sum(class_total_test)
    ...
    ut.numpy_to_mat(acc_array, 'Retention' + str(noise_aware) + get('Mapping_Strategy') + get('Quantization_Method'))
